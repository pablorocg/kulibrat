...
class GameState:
    ...
    def apply_move(self, move: Move) -> bool:
        ...
        if success:
            ...
            # CRITICAL FIX: DO NOT CHANGE PLAYERS HERE
            # Let the game engine handle player switching
            # The following line is commented out to prevent automatic switching
    ...
    def _apply_jump(self, move: Move) -> bool:
        ...
        while (0 <= row < 4 and 
               self.board[row, start_col] == opponent.value):
            line_length += 1
            row += direction
                        ...
                        if 0 <= new_col < 3:
                            if 0 <= new_row < 4:
                                ...
                                if self.is_empty((new_row, new_col)):
                                    moves.append(Move(MoveType.DIAGONAL, 
                                                        start_pos=(row, col),
                ...
                if self.board[row, col] == player_value:
                    ...
                    if 0 <= new_row < 4:
                        if self.board[new_row, col] == opponent_value:
                            moves.append(Move(MoveType.ATTACK, 
                                                start_pos=(row, col),
                                                end_pos=(new_row, col)))
        ...
        for row in range(4):
            for col in range(3):
                if self.board[row, col] == player_value:
                    ...
                    while (0 <= curr_row < 4 and 
                           self.board[curr_row, col] == opponent_value):
                        line_length += 1
                        curr_row += direction
                    ...
                    if 1 <= line_length <= 3:
                        ...
                        if 0 <= landing_row < 4:
                            if self.is_empty((landing_row, col)):
                                moves.append(Move(MoveType.JUMP, 
                                                    start_pos=(row, col),
