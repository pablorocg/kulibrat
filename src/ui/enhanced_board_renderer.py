...
class EnhancedBoardRenderer:
    ...
    def __init__(self, board_x: int, board_y: int, cell_size: int, 
                 board_rows: int = 4, board_cols: int = 3):
    ...
    def _load_resources(self):
        ...
        self._generate_glow_surface('black_glow', (100, 140, 255), self.piece_radius * 2)
        self._generate_glow_surface('select_glow', (255, 255, 150), self.piece_radius * 2)
    
    def _generate_wood_texture(self, name: str, base_color: Tuple[int, int, int], 
                             grain_color: Tuple[int, int, int], size: int = 256):
        ...
        for i in range(size):
            ...
            for j in range(size):
                if (i + j + noise) % 20 < 10:
                    ...
                    color = tuple(max(0, min(255, base_color[k] + darkness)) for k in range(3))
    ...
    def _add_wood_knot(self, texture: pygame.Surface, x: int, y: int, 
                      color: Tuple[int, int, int], radius: int = 10):
        """Add a knot pattern to the wood texture."""
        for i in range(x - radius, x + radius):
            for j in range(y - radius, y + radius):
                if (i - x)**2 + (j - y)**2 < radius**2 and 0 <= i < texture.get_width() and 0 <= j < texture.get_height():
                    # Radial darkening
                    ...
                    if dist < radius:
                        ...
                        new_color = tuple(max(0, min(255, pixel[k] + darkness)) for k in range(3))
    ...
    def render(self, screen: pygame.Surface, board: List[List[int]], 
    ...
    def _draw_board_background(self, screen: pygame.Surface):
        ...
        if self.use_textures and 'board_border' in self.textures:
            ...
            if border_texture.get_size() != (border_rect.width, border_rect.height):
                scaled_texture = pygame.transform.scale(border_texture, (border_rect.width, border_rect.height))
            else:
    ...
    def _draw_decorative_corners(self, screen: pygame.Surface, border_rect: pygame.Rect):
        ...
        corners = [
            ...
            (border_rect.right - corner_size, border_rect.bottom - corner_size)  # Bottom-right
        ]
        ...
        for row in range(self.board_rows):
            for col in range(self.board_cols):
                ...
                if self.use_textures and texture_name in self.textures:
                    ...
                    scaled_texture = pygame.transform.scale(texture, (self.cell_size, self.cell_size))
                ...
                if row == 0 or row == self.board_rows - 1:
                    marker_color = (100, 140, 255, 70) if row == 0 else (255, 100, 100, 70)
                    marker = pygame.Surface((self.cell_size, self.cell_size), pygame.SRCALPHA)
                    marker.fill(marker_color)
                    screen.blit(marker, cell_rect)
    
    def _highlight_valid_moves(self, screen: pygame.Surface, valid_moves: List[Tuple[int, int]]):
    ...
    def _draw_pieces(self, screen: pygame.Surface, board: List[List[int]], 
                   selected_pos: Optional[Tuple[int, int]] = None):
            ...
            for col in range(len(board[row]) if row < len(board) else 0):
                ...
                if piece_value != 0:  # If there's a piece here
                    ...
                    if is_selected and glow_name in self.glow_surfaces:
                        ...
                        scaled_glow = pygame.transform.scale(glow_surf, (scaled_size, scaled_size))
    ...
    def add_particles(self, x: int, y: int, particle_type: str, 
    ...
    def add_score_particles(self, row: int, col: int, is_black: bool):
        ...
        y = self.board_y + (0 if not is_black else self.board_rows - 1) * self.cell_size + self.cell_size // 2
    ...
    def animate_move(self, screen: pygame.Surface, board: List[List[int]], 
                   start_pos: Tuple[int, int], end_pos: Tuple[int, int], 
                   piece_value: int, callback=None):
        """
        ...
        if end_pos and 0 <= end_pos[0] < self.board_rows and 0 <= end_pos[1] < self.board_cols:
        ...
        if 0 <= start_row < len(temp_board) and 0 <= start_col < len(temp_board[start_row]):
        ...
        for frame in range(frames + 1):
            ...
            if self.use_lighting:
                ...
                pygame.draw.circle(
                    screen,
                    highlight_color,
                    (int(current_x + highlight_offset), int(current_y + highlight_offset)),
                    highlight_radius
                )
            ...
            if frame > 0:
                ...
                for i in range(1, blur_points + 1):
                    ...
                    if blur_progress >= 0:
                        ...
                        blur_surf = pygame.Surface((self.piece_radius * 2, self.piece_radius * 2), pygame.SRCALPHA)
                        pygame.draw.circle(
                            blur_surf,
                            blur_color,
