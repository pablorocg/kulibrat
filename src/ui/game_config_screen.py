...
class GameConfigScreen:
    ...
    def _create_background(self):
        ...
        for i in range(0, self.screen_width, dot_spacing):
            for j in range(0, self.screen_height, dot_spacing):
                ...
                if (i + j) % (dot_spacing * 2) == 0:
                    ...
                    dot_size = 2
                    pygame.draw.circle(bg, (230, 235, 245), (i, j), dot_size)
                elif (i + j) % (dot_spacing) == 0:
                    # Smaller dots for background texture
                    dot_size = 1
                    pygame.draw.circle(bg, (235, 240, 250), (i, j), dot_size)
        ...
        for y in range(self.screen_height):
            ...
            pygame.draw.line(
                gradient, (220, 230, 245, alpha), (0, y), (self.screen_width, y)
            )
                ...
                if event.type == pygame.QUIT:
                    ...
                    # Handle text input for active input field
                        ...
                        if event.key == pygame.K_BACKSPACE:
                            self.active_element["value"] = self.active_element["value"][
                                :-1
                            ]
                            ...
                            if event.unicode.isprintable():
                                ...
                                max_length = (
                                    30
                                    if self.active_element["id"] == "rl_model_path"
                                    else 20
                                )
                    ...
                    if event.button == 1:  # Left click
                        ...
                        if self.active_dropdown and not button_clicked:
                            dropdown_area = pygame.Rect(
                                ...
                                self.active_dropdown["rect"].height
                                + (len(self.active_dropdown["options"]) * 36),
                            )
                        ...
                        for slider in self.sliders:
                            ...
                            if slider_hit_rect.collidepoint(pos):
                                ...
                                pos_ratio = (pos[0] - slider["rect"].x) / slider[
                                    "rect"
                                ].width
                                pos_ratio = max(
                                    0, min(1, pos_ratio)
                                ...
                                if slider["step"] > 0:
                                    new_value = (
                                        round(new_value / slider["step"])
                                        * slider["step"]
                                    )

                                # Update slider value with range constraints
                                slider["value"] = min(
                                    max(new_value, slider["min"]), slider["max"]
                                )
                ...
                # Handle slider dragging with improved hit detection and range constraints
                    ...
                    for slider in self.sliders:
                        ...
                        if slider_drag_rect.collidepoint(event.pos):
                            ...
                            value_range = slider["max"] - slider["min"]
                            pos_ratio = (event.pos[0] - slider["rect"].x) / slider[
                                "rect"
                            ].width
                            pos_ratio = max(
                                0, min(1, pos_ratio)
                            ...
                            if slider["step"] > 0:
                                new_value = (
                                    round(new_value / slider["step"]) * slider["step"]
                                )
    ...
    def _setup_ui_elements(self):
        ...
        self.use_single_column = (
            panel_width < 540 or self.screen_width / self.screen_height < 1.0
        )
        ...
        if self.use_single_column:
            ...
            left_margin = right_margin = int(
                panel_width * 0.06
            )  # 6% of panel width for margins
            column_width = (
                panel_width - left_margin - right_margin - column_spacing
            ) // 2
        ...
        # FIX: Increased vertical spacing for the player name labels to prevent overlap
        label_offset = 24  # Space between label and input field

        # BLACK PLAYER (LEFT SIDE OR TOP IN SINGLE COLUMN)
        # Add a vertical offset to ensure labels don't overlap with section header
        input_y = content_y + header_spacing + 10  # Extra 10px to ensure no overlap

        # Black player name input with proper label positioning
        self.input_fields.append(
            {
                ...
                "rect": pygame.Rect(
                    panel_x + left_margin,
                    input_y + label_offset,
                    column_width,
                    element_height,
                ),
                "player": "black",
                "label_y": input_y,  # Store label position
            }
        )

        # Black player type dropdown
        dropdown_y = input_y + element_height + label_offset + base_spacing
        self.dropdowns.append(
            {
                ...
                "rect": pygame.Rect(
                    panel_x + left_margin,
                    dropdown_y + label_offset,
                    column_width,
                    dropdown_height,
                ),
                "player": "black",
                "label_y": dropdown_y,  # Store label position
            }
        )
        ...
        if self.use_single_column:
            ...
            red_section_y = (
                dropdown_y + dropdown_height + label_offset + section_spacing
            )
            self.red_header_pos = (panel_x + left_margin, red_section_y)

            # Red player name input below black player section with proper label positioning
            red_input_y = red_section_y + header_spacing + 10  # Extra spacing
            self.input_fields.append(
                {
                    "id": "red_player_name",
                    "type": "input",
                    ...
                    "rect": pygame.Rect(
                        panel_x + left_margin,
                        red_input_y + label_offset,
                        column_width,
                        element_height,
                    ),
                    "player": "red",
                    "label_y": red_input_y,  # Store label position
                }
            )

            # Red player type dropdown
            red_dropdown_y = red_input_y + element_height + label_offset + base_spacing
            self.dropdowns.append(
                {
                    "id": "red_player_type",
                    "label": "Player Type",
                    ...
                    "rect": pygame.Rect(
                        panel_x + left_margin,
                        red_dropdown_y + label_offset,
                        column_width,
                        dropdown_height,
                    ),
                    "player": "red",
                    "label_y": red_dropdown_y,  # Store label position
                }
            )

            # Set next section starting position
            next_section_y = (
                red_dropdown_y + dropdown_height + label_offset + section_spacing
            )
        else:
            # In two-column layout, place Red player in right column
            right_column_x = panel_x + panel_width - right_margin - column_width
            self.red_header_pos = (right_column_x +10, content_y)
        ...
        self.sliders.append(
            {
                ...
                "rect": pygame.Rect(
                    slider_x, score_slider_y, slider_width, slider_height
                ),
            }
        )
        ...
        self.sliders.append(
            {
                ...
                "rect": pygame.Rect(
                    slider_x, delay_slider_y, slider_width, slider_height
                ),
            }
        )
            ...
            self.input_fields.append(
                {
                    ...
                    "rect": pygame.Rect(
                        slider_x, rl_input_y, slider_width, element_height
                    ),
                }
            )
            ...
            self.buttons.append(
                {
                    ...
                    "type": "secondary",
                    "rect": pygame.Rect(
                        left_button_x, buttons_y, button_width, button_height
                    ),
                }
            )
            ...
            self.buttons.append(
                {
                    ...
                    "type": "primary",
                    "rect": pygame.Rect(
                        right_button_x, buttons_y, button_width, button_height
                    ),
                }
            )
    ...
    def _draw(self, screen: pygame.Surface):
        ...
        subtitle = self.FONTS["NORMAL"].render(
            "Game Configuration", True, self.COLORS["TEXT_ACCENT"]
        )
        subtitle_rect = subtitle.get_rect(
            centerx=self.panel_rect.centerx,
            centery=title_rect.bottom + 18 + subtitle_y_offset,
        )
        ...
        self._draw_section_header(
            ...
            align="left"
            if hasattr(self, "use_single_column") and self.use_single_column
            else "right",
        )
    ...
    def _draw_section_header(
        self, screen, text, position, color, shadow_color, align="left"
    ):
        ...
        shadow_rect = shadow_text.get_rect(
            topleft=(header_rect.x + 1, header_rect.y + 1)
        )
        ...
        for i in range(2):
            ...
            pygame.draw.line(
                screen, (*color[:3], alpha), line_start, line_end, 2 if i == 0 else 1
            )
    ...
    def _draw_input_field_background(self, screen, input_field):
        ...
        bg_color = (
            self.COLORS["INPUT_BG_FOCUS"] if is_active else self.COLORS["INPUT_BG"]
        )
        border_color = (
            self.COLORS["INPUT_BORDER_FOCUS"]
            if is_active
        ...
        shadow_rect.y += 2
        pygame.draw.rect(
            screen, self.COLORS["PANEL_SHADOW"], shadow_rect, border_radius=6
        )
    ...
    def _draw_input_field_content(self, screen, input_field):
        ...
        # Use the stored label position instead of calculating it
        label = self.FONTS["SMALL"].render(input_field["label"], True, font_color)
        label_y = input_field.get("label_y", input_field["rect"].y - 24)
        ...
        if text.get_width() > max_width:
            ...
            while visible_chars > 0:
                ...
                text = self.FONTS["NORMAL"].render(
                    clipped_text, True, self.COLORS["INPUT_TEXT"]
                )
                if text.get_width() <= max_width:
                    break
                visible_chars -= 1
        ...
        text_rect = text.get_rect(
            midleft=(input_field["rect"].x + 10, input_field["rect"].centery)
        )
    ...
    def _draw_dropdown_background(self, screen, dropdown):
        ...
        border_width = 2 if is_active else 1
        pygame.draw.rect(
            screen, border_color, dropdown["rect"], width=border_width, border_radius=6
        )
    ...
    def _draw_dropdown_content(self, screen, dropdown):
        ...
        text_rect = text.get_rect(
            midleft=(dropdown["rect"].x + 10, dropdown["rect"].centery)
        )
        screen.blit(text, text_rect)

        # Draw dropdown arrow with slight animation
        arrow_y_offset = (
            math.sin(math.radians(self.animation_counter)) * 1.0
            if self.active_dropdown == dropdown
            else 0
        )
        arrow_points = [
            (
                dropdown["rect"].right - 20,
                dropdown["rect"].centery - 5 + arrow_y_offset,
            ),
            (
                dropdown["rect"].right - 10,
                dropdown["rect"].centery + 5 + arrow_y_offset,
            ),
            (
                dropdown["rect"].right - 30,
                dropdown["rect"].centery + 5 + arrow_y_offset,
            ),
        ]
        ...
        if self.active_dropdown == dropdown:
            ...
            pygame.draw.rect(
                screen, self.COLORS["INPUT_BG"], dropdown_rect, border_radius=6
            )
            pygame.draw.rect(
                screen,
            ...
            for i, option in enumerate(dropdown["options"]):
                ...
                if option_rect.collidepoint(mouse_pos):
                    pygame.draw.rect(
                        screen,
                        self.COLORS["DROPDOWN_OPTION_HOVER"],
                        option_rect,
                        border_radius=4,
                    )
                ...
                option_text_rect = option_text.get_rect(
                    midleft=(option_rect.x + 26, option_rect.centery)
                )
    ...
    def _draw_slider(self, screen: pygame.Surface, slider: Dict[str, Any]):
        ...
        value_rect = value_display.get_rect(
            midright=(slider["rect"].right, slider["rect"].y - 24)
        )
        ...
        track_shadow.y += 2
        pygame.draw.rect(
            screen, self.COLORS["PANEL_SHADOW"], track_shadow, border_radius=6
        )

        # Draw main track
        pygame.draw.rect(
            screen, self.COLORS["SLIDER_TRACK"], slider["rect"], border_radius=6
        )

        # Calculate fill width based on value
        value_ratio = (slider["value"] - slider["min"]) / (
            slider["max"] - slider["min"]
        )
        fill_width = max(
            0, min(int(slider["rect"].width * value_ratio), slider["rect"].width)
        )
        ...
        if fill_width > 0:
            fill_rect = pygame.Rect(
                slider["rect"].x, slider["rect"].y, fill_width, slider["rect"].height
            )
        ...
        pygame.draw.circle(
            screen, self.COLORS["SLIDER_HANDLE"], (handle_x, handle_y), handle_radius
        )
        ...
        if slider["step"] >= 1.0:
            ...
            for i in range(tick_count):
                tick_x = slider["rect"].x + (
                    i * slider["rect"].width / (tick_count - 1)
                )
                ...
                pygame.draw.line(
                    screen, tick_color, (tick_x, tick_top), (tick_x, tick_bottom), 1
                )
    ...
    def _draw_checkbox(self, screen: pygame.Surface, checkbox: Dict[str, Any]):
        ...
        shadow_rect.y += 2
        pygame.draw.rect(
            screen, self.COLORS["PANEL_SHADOW"], shadow_rect, border_radius=4
        )

        # Draw checkbox box with proper state colors
        bg_color = (
            self.COLORS["INPUT_BG_FOCUS"] if is_hovered else self.COLORS["CHECKBOX_BG"]
        )
        border_color = (
            self.COLORS["INPUT_BORDER_FOCUS"]
            if is_hovered
            else self.COLORS["CHECKBOX_BORDER"]
        )

        pygame.draw.rect(screen, bg_color, checkbox["rect"], border_radius=4)
        pygame.draw.rect(
            screen, border_color, checkbox["rect"], width=2, border_radius=4
        )
        ...
        if checkbox["value"]:
            ...
            check_points = [
                (
                    checkbox["rect"].x + check_margin,
                    checkbox["rect"].centery + animation_offset,
                ),
                (
                    checkbox["rect"].centerx - 2,
                    checkbox["rect"].y
                    + checkbox["rect"].height
                    - check_margin
                    + animation_offset,
                ...
                (
                    checkbox["rect"].x + checkbox["rect"].width - check_margin,
                    checkbox["rect"].y + check_margin + animation_offset,
                ),
            ]
            ...
            pygame.draw.lines(
                screen,
                (*self.COLORS["CHECKBOX_CHECK"][:3], 60),
                False,
                shadow_points,
                3,
            )
            ...
            # Draw main checkmark
            pygame.draw.lines(
                screen, self.COLORS["CHECKBOX_CHECK"], False, check_points, 3
            )

        # Draw label with hover effect
        label_y_offset = 1 if is_hovered else 0
        label_color = (
            self.COLORS["TEXT_ACCENT"] if is_hovered else self.COLORS["TEXT_DARK"]
        )
        label = self.FONTS["NORMAL"].render(checkbox["label"], True, label_color)
        screen.blit(
            label,
            (
                checkbox["rect"].x + checkbox["rect"].width + 12,
                checkbox["rect"].centery - 10 + label_y_offset,
            ),
        )
    ...
    def _draw_button(self, screen: pygame.Surface, button: Dict[str, Any]):
        ...
        if button["type"] == "primary":
            bg_color = (
                self.COLORS["BUTTON_SECONDARY_HOVER"]
                if is_hovered
                else self.COLORS["BUTTON_SECONDARY"]
            )
        else:
            bg_color = (
                self.COLORS["BUTTON_PRIMARY_HOVER"]
                if is_hovered
                else self.COLORS["BUTTON_PRIMARY"]
            )
        ...
        text = self.FONTS["NORMAL"].render(
            button["label"], True, self.COLORS["BUTTON_TEXT"]
        )
        text_rect = text.get_rect(
            center=(button_rect.centerx, button_rect.centery + text_offset_y)
        )
