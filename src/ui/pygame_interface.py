"""
Simplified Pygame interface for the Kulibrat game.
"""
...
class KulibratGUI(GameInterface):
    ...
    def show_winner(self, winner: Optional[PlayerColor], game_state: GameState) -> None:
        ...
        if winner:
            winner_color = (
                self.colors["black_piece"] if winner == PlayerColor.BLACK 
        ...
        result_rect = result.get_rect(centerx=panel_x + panel_width // 2, y=panel_y + 80)
        ...
        # Draw buttons
        pygame.draw.rect(self.screen, self.colors["button"], play_again_rect, border_radius=5)
        pygame.draw.rect(self.screen, self.colors["red_piece"], quit_rect, border_radius=5)
        
        # Button text
        play_text = self.fonts["normal"].render("Play Again", True, self.colors["button_text"])
    ...
    def _draw_board(self):
        ...
        black_start_row = pygame.Surface((self.board_width, self.cell_size), pygame.SRCALPHA)
        black_start_row.fill(self.colors["start_row_black"])
        self.screen.blit(black_start_row, (self.board_x, self.board_y))
        
        red_start_row = pygame.Surface((self.board_width, self.cell_size), pygame.SRCALPHA)
        red_start_row.fill(self.colors["start_row_red"])
        self.screen.blit(red_start_row, (self.board_x, self.board_y + self.cell_size * (self.board_rows - 1)))
        ...
        for row in range(self.board_rows):
            for col in range(self.board_cols):
                ...
                cell_color = self.colors["board_light"] if (row + col) % 2 == 0 else self.colors["board_dark"]
                pygame.draw.rect(
                    self.screen,
                    cell_color,
        ...
        if self.waiting_for_move and self.selected_pos:
            ...
            # BLACK player potential scoring moves
            if current_player == PlayerColor.BLACK:
                scoring_row = 3  # RED's start row
                
                # Check for diagonal scoring moves
                diagonal_scoring_moves = []
                jump_scoring_moves = []
                
                for move in self.valid_moves:
                    if move.start_pos == self.selected_pos:
                        # Find diagonal scoring moves
                        if (move.move_type == MoveType.DIAGONAL and 
                            move.end_pos and move.end_pos[0] >= self.board_rows):
                            diagonal_scoring_moves.append(move)
                        
                        # Find jump scoring moves
                        elif (move.move_type == MoveType.JUMP and 
                            move.end_pos and move.end_pos[0] >= self.board_rows):
                            jump_scoring_moves.append(move)
                
                # Show diagonal scoring moves
                for move in diagonal_scoring_moves:
                    # Get target column by calculating offset from current column
                    col_diff = move.end_pos[1] - piece_col
                    target_col = piece_col + col_diff
                    ...
                    if 0 <= target_col < self.board_cols:
                        ...
                        # Draw attention-grabbing highlight
                        pygame.draw.rect(
                            self.screen,
                            (255, 215, 0, 180),  # Gold with transparency
                            (cell_x, cell_y, self.cell_size, self.cell_size)
                        )
                        
                        # Add "SCORE!" text
                        text = self.fonts["normal"].render("SCORE!", True, (255, 0, 0))
                ...
                # Show jump scoring moves
                for move in jump_scoring_moves:
                    # Jump moves are in the same column
                    ...
                    pygame.draw.rect(
                        self.screen,
                        (0, 200, 255, 180),  # Blue with transparency
                        (cell_x, cell_y, self.cell_size, self.cell_size)
                    )
                    
                    # Add "JUMP SCORE!" text
                    text = self.fonts["small"].render("JUMP SCORE!", True, (0, 0, 255))
                    text_rect = text.get_rect(center=(
                        cell_x + self.cell_size // 2,
                        cell_y + self.cell_size // 2
                    ))
            ...
            # RED player potential scoring moves
            elif current_player == PlayerColor.RED:
                scoring_row = 0  # BLACK's start row
                ...
                for move in self.valid_moves:
                    if move.start_pos == self.selected_pos:
                        # Find diagonal scoring moves
                        if (move.move_type == MoveType.DIAGONAL and 
                            move.end_pos and move.end_pos[0] < 0):
                            diagonal_scoring_moves.append(move)
        ...
        if self.waiting_for_move and self.selected_pos:
            valid_end_positions = [
                m.end_pos for m in self.valid_moves
                if m.start_pos == self.selected_pos and m.end_pos and
                0 <= m.end_pos[0] < self.board_rows and 0 <= m.end_pos[1] < self.board_cols
            ]
    ...
    def _draw_sidebar(self):
        ...
        sidebar_rect = pygame.Rect(
            sidebar_x, 0, self.sidebar_width, self.screen_height
        )
        ...
        current_player = self.current_game_state.current_player
        player_color = (
            self.colors["black_piece"] if current_player == PlayerColor.BLACK 
            else self.colors["red_piece"]
        )
        ...
        for i in range(4):
            piece_x = sidebar_x + self.sidebar_width // 2 - 3 * piece_spacing // 2 + i * piece_spacing
    ...
    def _draw_message(self):
        ...
        # Render message text
        message_text = self.fonts["normal"].render(
    ...
    def _handle_mouse_click(self, mouse_pos):
        ...
        if self.selected_pos is None:
            ...
            # CLAVE: Esta es la parte importante que tiene la lógica correcta para todos los movimientos
            valid_end_moves = [
                m for m in self.valid_moves
                if m.start_pos == self.selected_pos and (
                    # Para movimientos normales dentro del tablero
                    m.end_pos == pos or (
                        # Para movimientos diagonales de puntuación
                        m.move_type == MoveType.DIAGONAL and (
                            # BLACK diagonal scoring
                            (self.current_game_state.current_player == PlayerColor.BLACK and 
                            pos[0] == 3 and abs(pos[1] - piece_col) == 1) or
                            # RED diagonal scoring
                            (self.current_game_state.current_player == PlayerColor.RED and 
                            pos[0] == 0 and abs(pos[1] - piece_col) == 1)
                        )
                    ) or (
                        # Para movimientos de salto de puntuación (en la misma columna)
                        m.move_type == MoveType.JUMP and (
                            # BLACK jump scoring
                            (self.current_game_state.current_player == PlayerColor.BLACK and 
                            pos[0] == 3 and pos[1] == piece_col) or
                            # RED jump scoring
                            (self.current_game_state.current_player == PlayerColor.RED and 
                            pos[0] == 0 and pos[1] == piece_col)
                        )
                    )
                )
            ]
            ...
            if valid_end_moves:
                ...
                if self.selected_move.move_type == MoveType.DIAGONAL:
                    if ((self.current_game_state.current_player == PlayerColor.BLACK and pos[0] == 3) or
                        (self.current_game_state.current_player == PlayerColor.RED and pos[0] == 0)):
                ...
                if (
                    ...
                    self.selected_pos = None
                    self.show_message("Invalid move. Please select a piece or valid move.")
    ...
    def _show_rules(self):
        ...
        for line in rules_text:
            if (
                ...
                line.startswith("MOVES:") or
                line.startswith("SCORING:") or
                line.startswith("SPECIAL RULES:")
            ):
                text = self.fonts["normal"].render(
                    line, True, self.colors["text_dark"]
                )
                y_offset += 10
            else:
        ...
        close_button = pygame.Rect(
            panel_width // 2 - 50, panel_height - 60, 100, 40
        )
