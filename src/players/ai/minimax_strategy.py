...
class MinimaxStrategy(AIStrategy):
    ...
    def select_move(self, game_state: GameState, player_color: PlayerColor) -> Optional[Move]:
        ...
        for move in valid_moves:
            ...
            if self.use_alpha_beta:
                score, _ = self._alpha_beta(next_state, self.max_depth - 1, float('-inf'), float('inf'), 
                                          player_color.opposite(), player_color)
            else:
                score, _ = self._minimax(next_state, self.max_depth - 1, player_color.opposite(), player_color)
    ...
    def _minimax(self, state: GameState, depth: int, current_player: PlayerColor, 
                maximizing_player: PlayerColor) -> Tuple[float, Optional[Move]]:
        """
        ...
        if not valid_moves:
            return self._minimax(state, depth - 1, current_player.opposite(), maximizing_player)
        ...
        for move in valid_moves:
            ...
            score, _ = self._minimax(next_state, depth - 1, next_state.current_player, maximizing_player)
    ...
    def _alpha_beta(self, state: GameState, depth: int, alpha: float, beta: float,
                  current_player: PlayerColor, maximizing_player: PlayerColor) -> Tuple[float, Optional[Move]]:
        """
        ...
        # No valid moves means the other player gets an extra turn
        if not valid_moves:
            return self._alpha_beta(state, depth - 1, alpha, beta, current_player.opposite(), maximizing_player)
        ...
        if is_maximizing:
            ...
            for move in valid_moves:
                ...
                # Recursively evaluate
                score, _ = self._alpha_beta(next_state, depth - 1, alpha, beta, 
                                          next_state.current_player, maximizing_player)
