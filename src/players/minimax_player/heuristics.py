...
def advanced_minimax_optim_heuristic(
    state: GameState, player_color: PlayerColor
) -> float:
    """
        ...
        for col in range(state.BOARD_COLS):
            if state.board[row, col] == player_color.value:
                ...
                if player_color == PlayerColor.BLACK:
                    ...
                    progress = (
                        state.BOARD_ROWS - 1 - row
                    )  # RED wants to move up (decrease row)

                # Bonus for advancing pieces
                if (
                    player_color == PlayerColor.BLACK and row >= state.BOARD_ROWS // 2
                ) or (player_color == PlayerColor.RED and row < state.BOARD_ROWS // 2):
            ...
            # Subtract opponent progress
                ...
                progress_score -= (
                    opponent_progress * 0.7
                )  # Slightly less weight for opponent pieces
    ...
    for row in range(state.BOARD_ROWS):
        for col in range(state.BOARD_COLS):
            if state.board[row, col] == opponent_color.value:
                ...
                if (
                    0 <= forward_row < state.BOARD_ROWS
                    and state.board[forward_row, col] != 0
                ):
                ...
                for diagonal_col in [col - 1, col + 1]:
                    if 0 <= diagonal_col < state.BOARD_COLS:
                        ...
                        if (
                            0 <= diagonal_row < state.BOARD_ROWS
                            and state.board[diagonal_row, diagonal_col] != 0
                        ):
                            blocking_score += 0.6
    ...
    if len(player_moves) + len(opponent_moves) > 0:
        mobility_ratio = (len(player_moves) - len(opponent_moves)) / (
            len(player_moves) + len(opponent_moves)
        )

    mobility_component = (
        W4 * mobility_ratio * 10
    )  # Scale to similar magnitude as other components
...
def defensive_heuristic(state: GameState, player_color: PlayerColor) -> float:
    ...
    for p_row, p_col in player_pieces:
        prev_row = (
            p_row - opponent_direction
        )  # Position an opponent piece would be to attack
...
def strategic_heuristic(state: GameState, player_color: PlayerColor) -> float:
    ...
    for p_row, p_col in player_pieces:
        for jump_len in range(1, 4):  # Can jump over 1-3 pieces
            ...
            for i in range(1, jump_len + 1):
                ...
                if (
                    not (0 <= check_row < ROWS)
                    or state.board[check_row, p_col] != opponent_color.value
                ):
                    line_valid = False
                    break
    ...
    player_on_opponent_side = sum(
        1 for r, c in player_pieces if (r - opponent_start) * direction >= 0
    )
    opponent_on_player_side = sum(
        1 for r, c in opponent_pieces if (r - player_start) * direction <= 0
    )

    # Potential advantage for cycles if we have more pieces on enemy side
    if (
        player_on_opponent_side >= 2
        and player_on_opponent_side > opponent_on_player_side
    ):
        evaluation += 3.0
